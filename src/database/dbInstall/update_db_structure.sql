...
BEGIN
	IF TG_OP='UPDATE' THEN
		IF (OLD.activated IS DISTINCT FROM NEW.activated AND NEW.product_type = 'Native') THEN
			...
			WHERE i.productcode = NEW.productcode AND i.version = NEW.version;

			UPDATE products.process_product pp
			SET activated = NEW.activated
			WHERE pp.productcode = NEW.productcode AND pp.version = NEW.version;
			...
			WHERE (p.process_id) in (SELECT process_id
									 ...
									 WHERE pp.type = 'INPUT'
									   AND pp.productcode = NEW.productcode AND pp.version = NEW.version);
