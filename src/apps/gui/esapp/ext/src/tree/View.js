...
Ext.define('Ext.tree.View', {
    ...
    onRemove: function(ds, records, index) {
        ...
        if (me.viewReady) {
            ...
            if (me.hasListeners.itemremove) {
                for (i = records.length - 1, index += i; i >= 0; --i, --index) {
                    // Note that this event signature is different from what we do in 6.0+
    ...
    onItemClick: function(record, item, index, e) {
        if (e.getTarget(this.expanderSelector, item) && record.isExpandable()) {
            ...
            // toggle causes the picker view to refreshSize which scrolls the view
            // to top if the picker has autoHeight, if that happens we need to scroll
            // back to the record that caused this toggle.
            if(this.getSizeModel().height.auto) {
                this.scrollRowIntoView(this.getRow(record));
            }
            return false;
        }
        return this.callParent([record, item, index, e]);
