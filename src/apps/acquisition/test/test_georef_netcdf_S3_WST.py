...
input_dir='/data/processing/exchange/Sentinel-3/S3A_SL_2_WST/S3A_SL_2_WST____20180410T101307_20180410T101607_20180410T114512_0179_030_036_4860_MAR_O_NR_003.SEN3/'
output_dir='/data/processing/exchange/Sentinel-3/Outputs/Images/S3A_SL_2_WST/'


band='20180410101307-MAR-L2P_GHRSST-SSTskin-SLSTRA-20180410114512-v02.0-fv01.0.nc'
geo='20180410101307-MAR-L2P_GHRSST-SSTskin-SLSTRA-20180410114512-v02.0-fv01.0.nc'
...
def geo_ref_s3_data(bandname=None):
    ...
    un_proj_filename = bandname + '_un_proj.tif'
    output_file = output_dir + un_proj_filename
    output_driver = gdal.GetDriverByName('GTiff')
    orig_size_x = reflectance.shape[2]
    orig_size_y = reflectance.shape[1]
    in_data_type = gdal.GDT_Float32
    output_ds = output_driver.Create(output_file, orig_size_x, orig_size_y, 1, in_data_type)
    for i, image in enumerate(reflectance, 1):
    ...
    x_size = 0.00892857
    y_size = 0.00892857
    write_vrt(un_proj_filename=un_proj_filename, rasterXSize=orig_size_x, rasterYSize=orig_size_y)

    input_vrt = output_dir + 'seasurfacetemp.vrt'
    output_tif = output_dir + bandname + '.tif'

    command = 'gdalwarp -te {} {} {} {} -s_srs "epsg:4326" -r near -t_srs "+proj=longlat +datum=WGS84" -ot Float32 {} {}'.format(
        lon_min, lat_min, lon_max, lat_max, input_vrt, output_tif)
    # command = 'gdalwarp -te {} {} {} {} -s_srs "epsg:4326" -tr {} {} -r near -t_srs "+proj=longlat +datum=WGS84" -ot Float32 {} {}'.format(
    #     lon_min, lat_min, lon_max, lat_max, x_size, y_size, input_vrt, output_tif)
...
def write_vrt(un_proj_filename=None, rasterXSize=None, rasterYSize=None):
    # Write the 'vrt' file
...
geo_ref_s3_data(bandname='sea_surface_temperature')
