...
def get_list_current_subdirs_ftp(remote_url, usr_pwd):
    ...
    if usr_pwd != ':':
        d.setopt(pycurl.USERPWD, str(usr_pwd))
...
######################################################################################
#   build_list_matching_files_tmpl_mars
#   Purpose: return the list of file names matching a 'template' with 'date' placeholders
#            It is the entry point for the 'http_templ' source type
#   Author: Marco Clerici, JRC, European Commission
#   Date: 2015/02/18
...
#           frequency: dataset 'frequency' (see DB 'frequency' table)
#
def build_list_matching_files_tmpl_mars(base_url, template, from_date, to_date, frequency_id, usr_pwd):
    # Add a check on frequency
    try:
        frequency = datasets.Dataset.get_frequency(frequency_id, datasets.Frequency.DATEFORMAT.DATETIME)
    except Exception as inst:
    ...
    try:
        ...
        indicator_name = str(parameters.get('indicator_name'))
        indicator_group = str(parameters.get('indicator_group'))
        level = str(parameters.get('level'))
        for date in dates:
            dekad = date.strftime("%Y%m%d")
            parameter = 'asap/getIndicatorsInfo.php?dekad=' + dekad + '&indicator_name=' + indicator_name + '&indicator_group=' + indicator_group + '&level=' + level
            remote_url_file = base_url +os.path.sep+ parameter

            json_list = get_json_from_url(remote_url_file, userpwd=usr_pwd)
            if len(json_list) == 1:
                location = json_list.get(dekad).get(indicator_name).get('location')
                file_dir= str(os.path.dirname(location))
                filename_img = str(os.path.basename(location))
                filename_hdr = filename_img.split('.')[0] + '.hdr'
                list_filenames.append(file_dir+os.path.sep+filename_img)
                list_filenames.append(file_dir+os.path.sep+filename_hdr)

    except Exception as inst:
        logger.debug("Error in Creation of file lists")
        raise
...
######################################################################################
#   build_list_dates_generic
#   Purpose: return the list dates placeholders
#   Author: Vijay Charan Venkatachalam, JRC, European Commission
#   Date: 2021/06/16
#   Inputs: from_date: start date for the dataset (datetime.datetime object)
#           to_date: end date for the dataset (datetime.datetime object)
#           frequency: dataset 'frequency' (see DB 'frequency' table)
                        ...
                        if execute_trigger:
                            ...
                            if internet_type == 'ftp' or internet_type == 'http':
                                ...
                                try:
                                    ...
                                    logger.error("Error in creating current lists. Continue")
                                    b_error = True
                                    continue

                            elif internet_type == 'http_tmpl_mars':
                                # Create the full filename from a 'template' which contains
                                try:
                                    current_list = build_list_matching_files_tmpl_mars(str(internet_source.url),
                                                                                  internet_source.include_files_expression,
                                                                                  internet_source.start_date,
                                                                                  internet_source.end_date,
                                                                                  str(internet_source.frequency_id),
                                    ...
                                    logger.error("Error in creating curent lists. Continue")
                                    b_error = True
                                    continue
                                        ...
                                        for filename in list(listtoprocess):
                                            ...
                                            try:
                                                ...
                                                #                                            target_dir=es_constants.ingest_dir)
                                                    ...
                                                    #
                                                    download_link = internet_source.url+ filename
                                                    result = get_file_from_url(str(download_link),
                                                                               target_dir=es_constants.ingest_dir,
                                                                               target_file=os.path.basename(
                                                                                   filename),
