...
def processing_std_spi_monthly(res_queue, pipeline_run_level=0,pipeline_printout_level=0,
    ...
    if pipeline_run_level > 0:
        ...
        pipeline_run(verbose=pipeline_run_level, logger=spec_logger, log_exceptions=spec_logger, history_file='/eStation2/log/.ruffus_history_spi_monthly.sqlite',\
                     checksum_level=0)
        spec_logger.info("After running the pipeline %s" % 'processing_std_precip_spi_monthly')

    if pipeline_printout_level > 0:
        pipeline_printout(verbose=pipeline_printout_level, output_stream=fwrite_id, history_file='/eStation2/log/.ruffus_history_spi_monthly.sqlite',\
                     checksum_level=0)
