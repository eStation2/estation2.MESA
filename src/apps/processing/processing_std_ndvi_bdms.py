...
def create_pipeline(prod, starting_sprod, mapset, version, starting_dates_linearx2=None, proc_lists=None,
    ...
    formatter_in = "(?P<YYYYMMDD>[0-9]{8})" + in_prod_ident_linearx2
    formatter_out = [
        "{subpath[0][5]}" + os.path.sep + subdir_baresoil_linearx2 + "{YYYYMMDD[0]}" + prod_ident_baresoil_linearx2]
    ...
    def vgt_ndvi_vci(input_file, output_file):
        ...
        output_file = functions.list_to_element(output_file)
        functions.check_output_dir(os.path.dirname(output_file))
        args = {"input_file": input_file[0], "max_file": input_file[1], "min_file": input_file[2],
                "output_file": output_file, "output_format": 'GTIFF', "options": "compress = lzw"}
...
def processing_std_ndvi(res_queue, pipeline_run_level=0, pipeline_printout_level=0,
    ...
    if pipeline_run_level > 0:
        pipeline_run(verbose=pipeline_run_level, logger=spec_logger, touch_files_only=touch_files_only,
                     history_file=os.path.join(es_constants.log_dir, '.ruffus_history.sqlite'))
