...
class TestMapSet(unittest.TestCase):
    ...
    def test_compute_pixel_area(self):
        ...
        self.assertAlmostEqual(my_mapset.pixel_area, 981271.687058,delta=0.001)

        n_line = 3256
        n_col = 2913
        my_mapset.compute_pixel_area(n_line, n_col)
        self.assertAlmostEqual(my_mapset.pixel_area, 969693.928995,delta=0.001)

        n_line = 1000
        n_col = 1000
        my_mapset.compute_pixel_area(n_line, n_col)

        self.assertAlmostEqual(my_mapset.pixel_area, 860363.323192,delta=0.001)
