...
class TestFunctions(unittest.TestCase):
    ...
    def test_files_temp_ajacent(self):
        ...
        file_t0 = es_constants.es2globals['test_data_dir']+'/tamsat-rfe/3.0/TAMSAT-Africa-4km/tif/10d/20190101_tamsat-rfe_10d_TAMSAT-Africa-4km_3.0.tif'
        file_t1 = es_constants.es2globals['test_data_dir']+'/tamsat-rfe/3.0/TAMSAT-Africa-4km/tif/10d/20190111_tamsat-rfe_10d_TAMSAT-Africa-4km_3.0.tif'
    ...
    def test_getJRCRefSettings(self):
        ...
        result = False
        if 'version' in jrc_ref_settings.keys():
            result = True
        self.assertIsInstance(jrc_ref_settings, dict)
        self.assertTrue(result)
    ...
    def test_is_file_exists_in_path(self):
        file_path = es_constants.es2globals['test_data_dir']+'/tamsat-rfe/3.0/TAMSAT-Africa-4km/tif/10d/20190101_tamsat-rfe_10d_TAMSAT-Africa-4km_3.0.tif'
    ...
    def test_is_float(self):
        ...
        floatvar2 = ''
        if version == 2:
            result1 = functions.is_float(floatvar1)
            result2 = functions.is_float(floatvar2)
        if version == 3:
            result1 = functions.str_is_float(floatvar1)
            result2 = functions.str_is_float(floatvar2)
    ...
    def test_is_int(self):
        ...
        floatvar2 = ''
        if version == 2:
            result1 = functions.is_int(floatvar1)
            result2 = functions.is_int(floatvar2)
        if version == 3:
            result1 = functions.str_is_int(floatvar1)
            result2 = functions.str_is_int(floatvar2)
    ...
    def test_row2dict(self):
        categories_dict_all = []
        if version == 2:
            categories = querydb.get_categories(all=True)
        if version == 3:
    ...
    @unittest.skipIf(install_type == 'server', "Test NOT executed on mesa-proc - cannot reach out")
    ...
    def test_save_read_netcdf_scaling(self):
        ...
        nc_file = es_constants.es2globals['test_data_dir']+'/modis-sst/native/AQUA_MODIS.20200320.L3m.DAY.SST.sst.4km.NRT.nc'
