...
class NCOLCIGeo(BaseFileHandler):
	...
	# M.C. 05.02.2017 -> geo_coordinates.nc of S3A_OL_2_WRR does not contain _FillValues
	try: 
	        ...
	        ds = (np.ma.masked_equal(variable[:], -2147483648) *
        	      (variable.attrs['scale_factor'] * 1.0) +
        	      variable.attrs.get('add_offset', 0))
...
class NCOLCI2Prod(BaseFileHandler):

    def __init__(self, filename, filename_info, filetype_info):
        super(NCOLCI2Prod, self).__init__(filename, filename_info,
                                       filetype_info)
        self.nc = h5netcdf.File(filename, 'r')
        self.channel = filename_info['dataset_name']
        cal_file = os.path.join(os.path.dirname(
...
class NCOLCI2Refl(BaseFileHandler):

    # Copied from NCOLCI1B and adapted (M.C. 01/03/2018)

    def __init__(self, filename, filename_info, filetype_info):
        super(NCOLCI2Refl, self).__init__(filename, filename_info,
                                       filetype_info)
        self.nc = h5netcdf.File(filename, 'r')
        self.channel = filename_info['dataset_name']
        cal_file = os.path.join(os.path.dirname(
    ...
    def get_dataset(self, key, info):
        ...
        if self.channel != key.name:
            return
        logger.debug('Reading %s.', key.name)
        variable = self.nc[self.channel + '_reflectance']

        # Debug
        # print variable.attrs['_FillValue']
        # print variable.attrs['scale_factor']
        # print variable.attrs['add_offset']
        # print variable.attrs['units'] - Not defined in .nc
        # print self.cal['solar_flux'][:]
        # print self.cal['detector_index'][:]
        # print self.cal['detector_index'].attrs['_FillValue']
